...
supprimer_window = Tk()
supprimer_window.geometry('1200x600')
supprimer_window.title("Projet PR215")
...
def supprimer():
    code_pays = code_txt.get()
    data_file.pop(str(code_pays))
    json.dump(data_file, open('./my_data.json', 'w'))
    ...
    messagebox.showinfo('Succes', 'Suppression du pays effectue avec succes')
...
frame = Frame(supprimer_window, bg="#6AADB6", padx=70, pady=30)
...
s = ttk.Style()
style = ttk.Style(supprimer_window)
...
supprimer_btn = Button(frame, text="Supprimer", padx=10,
                       pady=10, fg="white", bg="#263D42", command=lambda: supprimer())
supprimer_btn.grid(row=3, column=1, pady=10)
